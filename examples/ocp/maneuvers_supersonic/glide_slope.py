...
def get_glide_slope(_g0, _m, _s_ref, _eta, _e_vals: Optional[np.array] = None):
    if _e_vals is None:
        _e_min = 0.5 * (0.3 * speed_of_sound(0.)) ** 2
        _e_max = 0.5 * (3.0 * speed_of_sound(0.)) ** 2
    ...
    v_interp = sp.interpolate.pship(_e_vals, _v_vals)

    for idx, (_v_val, _e_val, _h_val) in enumerate(zip(_v_vals, _e_vals, _h_vals)):
        _v_pert = _v_val + 10
        _dh_dv = (h_interp(_v_pert) - h_interp(_v_val)) / (_v_pert - _v_val)
