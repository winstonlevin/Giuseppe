...
from x15_atmosphere import dens_fun, sped_fun, g0
...
cla = np.asarray(cla_fun(mach), dtype=float).flatten()
cd0 = np.asarray(cd0_fun(mach), dtype=float).flatten()
cdl = np.asarray(cdl_fun(mach), dtype=float).flatten()
...
lift = qdyn * s_ref * CL
drag = qdyn * s_ref * CD
alpha_mg = weight / (qdyn * s_ref * cla) * np.cos(x_dict['gam']) / np.cos(u_dict['phi'])
alpha_ld = (cd0 / cdl) ** 0.5 / cla
alpha_opt = lam_dict['gam'] / (2 * cdl * cla * x_dict['v'] * lam_dict['v'])
ld_mg = cla * alpha_mg / (cd0 + cdl * (cla * alpha_mg) ** 2)
ld_max = cla * alpha_ld / (cd0 + cdl * (cla * alpha_ld) ** 2)

# Glide Slope
h_interp, v_interp, gam_interp, drag_interp = get_glide_slope(
    x_dict['m'][0], _h_min=0., _h_max=130e3, _mach_min=0.25, _mach_max=7.
)
