...
def get_glide_slope(e_vals: Optional[np.array] = None,
    ...
    def calc_zo_dict(_e, _h, _gam=None):
        ...
        _dict['drag'] = _dict['CD0'] * _dict['qdyn_s_ref'] + _dict['CD1'] * _dict['lift'] \
            + _dict['CD2'] / _dict['qdyn_s_ref'] * _dict['lift'] ** 2
        _dict['ddrag_dlift'] = _dict['CD1'] + 2 * _dict['CD2'] / _dict['qdyn_s_ref'] * _dict['lift']
    ...
    for idx, e_val in enumerate(e_vals):
        ...
        if flag != _f_zero_converged_flag or h_sol > h_max_i:
            ...
            for jdx, h_search_val in enumerate(h_search_vals):
                fzero_search_vals[jdx] = fzero(e_val, h_search_val)
    ...
    if correct_gam and len(e_vals[valid_idces]) > 2:
        ...
        gam_vals = - np.arcsin(np.maximum(np.minimum(_zo_dict['drag'] / mass * dh_de_interp(e_vals), 1.), -1.))
...
if __name__ == '__main__':
    ...
    v_vals_flat = glide_dict_flat['v']
    mach_vals_flat = v_vals_flat / np.asarray(sped_fun(h_vals_flat)).flatten()
    ...
    ax_mach = fig_glide_slope.add_subplot(224)
    ax_mach.plot(e_vals, mach_vals, label='Sphere')
    ax_mach.plot(e_vals_flat, mach_vals_flat, label='Flat')
    # ax_mach.plot(e_vals_es, mach_vals_es * r2d, '--', label='min(DR)')
    ax_mach.set_xlabel(e_lab)
