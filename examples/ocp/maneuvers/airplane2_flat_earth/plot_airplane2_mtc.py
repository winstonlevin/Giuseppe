...
from airplane2_aero_atm import g, s_ref, CL0, CLa_fun, CD0_fun, CD1, CD2_fun, max_ld_fun,\
    sped_fun, dens_fun
...
if DATA == 0:
    with open('guess_mtc.data', 'rb') as f:
        sol = pickle.load(f)
        sol.cost = np.nan
elif DATA == 1:
    with open('seed_sol_mtc.data', 'rb') as f:
        sol = pickle.load(f)
else:
...
mass = x_dict['m_nd'] * k_dict['m_scale']
...
# PLOT STATES
ylabs = (r'$h$ [1,000 ft]', r'$V$ [1,000 ft/s]', r'$\gamma$ [deg]', r'$m$ [lbm]')
ymult = np.array((1e-3 * k_dict['h_scale'], 1e-3 * k_dict['v_scale'], r2d, g * k_dict['m_scale']))
...
if PLOT_COSTATE:
    # PLOT COSTATES
    ylabs = (
        r'$\lambda_{h}$', r'$\lambda_{V}$', r'$\lambda_{\gamma}$', r'$\lambda_{m}$'
    )
    ymult = np.array((1. / k_dict['h_scale'], 1. / k_dict['v_scale'], 1., 1. / k_dict['m_scale']))
