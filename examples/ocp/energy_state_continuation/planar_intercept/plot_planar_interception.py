...
for idx, (x, lam) in enumerate(zip(sol.x, sol.lam)):
    ax_x_lam[idx][0].grid()
    ax_x_lam[idx][1].grid()
    ax_x_lam[idx][0].plot(sol.t, x, label='SciPy')
    ax_x_lam[idx][1].plot(sol.t, lam, label='SciPy')
...
ax_xy.plot(sol.x[0, :], sol.x[1, :], label='SciPy')
fig_xy.tight_layout()
...
ax_u.set_xlabel(r'$t$')
ax_u.set_ylabel(r'$u$')
ax_u.plot(sol.t, sol.u[0, :], label='SciPy')
fig_u.tight_layout()
